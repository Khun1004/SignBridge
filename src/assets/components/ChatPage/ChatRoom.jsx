...
const OFFICIAL_ROOMS = [
    { id:'official_signbridge', name:'SignBridge Official', sub:'공식 커뮤니티', avatar:'🤟', description:'SignBridge 공식 채팅방입니다. 공지사항, 수어 팁, 커뮤니티 소식을 공유해요.', memberCount:1284, isOfficial:true, isGroup:true, previewMessages:[
            ...
            { id:'p3', name:'민지',       email:'minji@sb.com',  text:'수어 인식이 더 정확해졌으면 좋겠어요 📚', at: new Date(Date.now()-3600000).toISOString() },
        ]},
    { id:'official_learners', name:'수어 배우기 방', sub:'학습자 모임', avatar:'📚', description:'수어를 배우는 분들을 위한 방입니다.', memberCount:437, isOfficial:true, isGroup:true, previewMessages:[
            ...
            { id:'p2', name:'소라', email:'sora@sb.com',  text:'"감사합니다"를 수어로 어떻게 하나요?',       at: new Date(Date.now()-5400000).toISOString() },
        ]},
]
...
function Avatar({ name, photo, size=40, radius='50%', bg='linear-gradient(135deg,#7C3AED,#A78BFA)', className='' }) {
    ...
    return (
        <div className={`sb-av ${className}`} style={{
            ...
            <div className="cw-fp-body">
                {cats.map((cat,ci)=>(
                    <div key={ci} className="cw-fp-section">
                        ...
                        <div className="cw-fp-grid">{cat.emojis.map(e=><button key={e} className="cw-fp-btn nd" onClick={()=>{onSelect(e);onClose()}}>{e}</button>)}</div>
                    </div>
                ))}
            ...
            <div className="cw-member-list">
                {all.map(m=>(
                    <div key={m.email} className="cw-member-row">
                        ...
                        <div className="cw-member-info"><span className="cw-member-name">{m.name||m.email}</span>{m.role&&<span className="cw-member-role">{m.role}</span>}</div>
            ...
            <div className="cw-msgsearch-results">
                ...
                {results.map(m=>(
                    <div key={m.id} className="cw-msgsearch-row" onClick={()=>{onJump(m.id);onClose()}}>
                        ...
                        <div className="cw-msgsearch-snippet">{(m.text||m.fileName||'').split(new RegExp(`(${q})`,'gi')).map((p,i)=>p.toLowerCase()===q.toLowerCase()?<mark key={i}>{p}</mark>:p)}</div>
            ...
            <div className="ci-modal nd" onClick={e=>e.stopPropagation()}>
                ...
                <div className="ci-modal-body">
                    ...
                    <div className="ci-emoji-grid">
                        {AVATAR_EMOJIS.map(e=><button key={e} className={`ci-emoji-opt nd ${photoInput===e?'selected':''}`} onClick={()=>setPhoto(photoInput===e?'':e)}>{e}</button>)}
                    ...
                    <div className="ci-modal-field">
                        ...
                        <input className="ci-modal-input nd" value={nickInput} onChange={e=>setNick(e.target.value)} onKeyDown={e=>e.key==='Enter'&&doSave()} placeholder="닉네임 입력"/>
                    </div>
...
function RoomMenu({ room, onMarkRead, onMute, onDelete, onBlock, onClose }) {
    ...
    const item=(icon,label,onClick,danger=false)=>(
        <button className={`ci-menu-item nd ${danger?'ci-menu-danger':''}`} onClick={e=>{e.stopPropagation();onClick();onClose()}}>
                ...
                <div className="ci-gp-messages">
                    ...
                    {(room.previewMessages||[]).map(msg=>(
                        <div key={msg.id} className="ci-gp-msg-row">
                            ...
                            <div className="ci-gp-msg-body"><div className="ci-gp-msg-name">{msg.name}</div><div className="ci-gp-msg-text">{msg.text}</div></div>
                ...
                <div className="cw-fwd-list">
                    {all.length===0?(
                        ...
                        <div key={r.id} className="cw-fwd-room nd">
                            <Avatar name={r.avatar||r.name} photo={r.avatar?.length===2?r.avatar:undefined} size={44}/>
                            <div className="cw-fwd-room-info"><div className="cw-fwd-room-name">{r.name}</div>{r.sub&&<div className="cw-fwd-room-sub">{r.sub}</div>}</div>
        ...
        <div className="cw-img-preview-overlay nd" onClick={onCancel}>
            <div className="cw-img-preview-modal nd" onClick={e=>e.stopPropagation()}>
                ...
                <div className="cw-img-preview-body">
                    ...
                    <div className="cw-img-preview-meta"><span className="cw-img-preview-name">{file.name}</span><span className="cw-img-preview-size">{fmtFileSize(file.size)}</span></div>
                </div>
                    ...
                    <div className={`cw-row ${isMe?'cw-me':'cw-them'} ${isPinned?'cw-row-pinned':''}`}>
                        {!isMe&&(
                            <div className="cw-avatar">
                                {isGroup
                                    ...
                                    : room.avatar && room.avatar.length <= 2
                                        ? room.avatar
                                        : (senderName||room.name||'?').charAt(0).toUpperCase()
                                }
                            </div>
                        )}
                        <div className="cw-bwrap">
                            ...
                            <div className="cw-bubble-wrap"
                                 onMouseEnter={()=>{ clearTimeout(hoverTimer.current); setHoveredMsgId(msg.id) }}
                                 onMouseLeave={()=>{ hoverTimer.current=setTimeout(()=>setHoveredMsgId(null),200) }}>
                                <div className={`cw-bubble ${isMe?'cw-bubble-me':'cw-bubble-them'}`}>
                                    ...
                                    {msg.isImage&&msg.fileUrl?(<div className="cw-img-wrap"><img src={msg.fileUrl} alt={msg.fileName} className="cw-img"/><div className="cw-img-name">{msg.fileName}</div></div>)
                                        :msg.fileName?(<div className="cw-file"><div className="cw-file-icon">📎</div><div className="cw-file-info"><div className="cw-file-name">{msg.fileName}</div></div></div>)
                                            :msg.text}
                                </div>
                            ...
                            {aggR.length>0&&(
                                <div className={`cw-reactions ${isMe?'reactions-left':'reactions-right'}`}>
                                    {aggR.map(([e,count])=><span key={e} className={`cw-reaction-chip nd ${myR===e?'cw-reaction-mine':''}`} onClick={ev=>{ev.stopPropagation();addReaction(msg.id,e)}}>{e} {count}</span>)}
                                    <button className="cw-who-btn nd" onClick={ev=>{ev.stopPropagation();setPopover(isWhoOpen?null:{msgId:msg.id,type:'whoReacted'})}}><IconMembers/></button>
                                    {isWhoOpen&&<div onClick={e=>e.stopPropagation()}><WhoReacted reactions={msg.reactions} nameMap={nameMap} onClose={closePopover}/></div>}
                                </div>
                            )}
                        </div>
            ...
            <div className="cw-window" style={{left:pos.x,top:pos.y}}>
                ...
                <div className="cw-header" onMouseDown={onMouseDown}>
                    <div className="cw-header-left">
                        ...
                        <div>
                            ...
                            <div className="cw-hsub">{isGroup?`${fmtMembers(room.memberCount)}명 참여 중`:room.sub||'온라인'}</div>
                        </div>
                    </div>
                ...
                {replyTo&&!editingMsg&&(
                    <div className="cw-reply-bar nd">
                        <div className="cw-reply-bar-content"><IconReply/><div className="cw-reply-bar-info"><span className="cw-reply-bar-name">{replyTo.name}에게 답장</span><span className="cw-reply-bar-text">{replyTo.fileName?`📎 ${replyTo.fileName}`:replyTo.text}</span></div></div>
...
export default function ChatRoom({ onClose, myEmail='', myName='', profile=null, communityProfile=null, initialRoom=null }) {
    ...
    const myChatId    = Array.isArray(communityProfile)
        ? communityProfile[0]?.chatId||null
        : communityProfile?.chatId||null
            ...
            <div className="ci-window" style={{left:pos.x,top:pos.y}}>
                ...
                <div className="ci-rail nd">
                    <div className="ci-rail-logo">
                        <img src="/SignBridge.png" alt="SB" className="ci-rail-logo-img" onError={e=>{e.target.style.display='none';e.target.nextElementSibling.style.display='flex'}}/>
                        <div className="ci-rail-logo-fallback" style={{display:'none'}}>🤟</div>
                    </div>
                    <nav className="ci-rail-nav">
                        {[
                            {id:'profile',  icon:<IconUser/>,  label:'프로필'},
                            {id:'chats',    icon:<IconChat/>,  label:'채팅',   badge:unreadTotal},
                            {id:'groups',   icon:<IconGroup/>, label:'그룹'},
                            {id:'starred',  icon:<span style={{fontSize:16}}>⭐</span>, label:'즐겨찾기', badge:starred.length},
                        ].map(({id,icon,label,badge})=>(
                            <button key={id} className={`ci-rail-tab nd ${tab===id?'active':''}`} onClick={()=>setTab(id)}>
                                <span className="ci-rail-tab-icon-wrap">
                                    {icon}
                                    {badge>0&&<span className="ci-rail-badge">{badge}</span>}
                                </span>
                    ...
                    {tab==='profile'&&(
                        <div className="ci-pane">
                            <div className="ci-pane-hd" onMouseDown={onMouseDown}>
                                <div className="ci-pane-hd-avatar nd" onClick={()=>setShowPhotoModal(true)}>
                                    {photo
                                        ? <span style={{fontSize:22}}>{photo}</span>
                                        : <span style={{fontSize:18,fontWeight:700,color:'#8a6c2a'}}>{displayName.charAt(0)}</span>
                                    }
                                </div>
                            ...
                            <div className="ci-pane-scroll nd">
                                <div className="ci-prof-hero nd" onClick={()=>setShowPhotoModal(true)}>
                                    <div className="ci-prof-hero-av">
                                        {photo?<span style={{fontSize:28}}>{photo}</span>:<span>{displayName.charAt(0)}</span>}
                                        <div className="ci-prof-hero-cam">📷</div>
                                    </div>
                                    <div className="ci-prof-hero-info">
                                        <div className="ci-prof-hero-nick">{nickname||displayName}</div>
                                        <div className="ci-prof-hero-email">{displayEmail}</div>
                                    </div>
                                    <div className="ci-prof-hero-arrow">›</div>
                                </div>
                                {myChatId&&(
                                    <div className="ci-chatid-banner">
                                        <span className="ci-chatid-at">@</span>
                                        <span className="ci-chatid-val">{myChatId}</span>
                                        <span className="ci-chatid-badge">커뮤니티 ID</span>
                                    </div>
                                )}
                                ...
                                <div className="ci-info-card">
                                    {[
                                        ...
                                        ['수어와의 관계',profile?.preferredSign||'-'],
                                        ...(profile?.orgType&&profile.orgType!=='personal'?[['기관명',profile?.officeName||'-'],['기관 코드',profile?.orgCode||'-']]:[] ),
                                    ].map(([k,v])=>(
                                ...
                                <div className="ci-info-card" style={{marginBottom:20}}>
                                    {[['우편번호',profile?.zonecode||'-'],['주소',profile?.address||'-'],['상세주소',profile?.addressDetail||'-']].map(([k,v])=>(
                                        <div className="ci-info-row" key={k}><span className="ci-info-key">{k}</span><span className="ci-info-val">{v}</span></div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="ci-pane">
                            ...
                            <div className="ci-pane-scroll nd">
                                ...
                                {OFFICIAL_ROOMS.map(room=>(
                                    <OfficialRoomBanner key={room.id} room={room} joined={joinedGroups.includes(room.id)} onJoin={()=>joinGroup(room.id)} onLeave={()=>leaveGroup(room.id)} onOpen={()=>openChat(room,true)} onPreview={()=>setPreviewRoom(room)}/>
                                ))}
                    ...
                    {tab==='starred'&&(
                        <div className="ci-pane">
                            <div className="ci-pane-hd nd" onMouseDown={onMouseDown}>
                                ...
                                <span className="ci-pane-title">즐겨찾기</span>
                                <button className="ci-pane-close nd" onClick={onClose}>✕</button>
                            </div>
                            <div className="ci-pane-scroll">
                                {starred.length===0 ? (
                                    <div className="ci-empty" style={{padding:'48px 20px'}}>
                                        <div className="ci-empty-icon">⭐</div>
                                        <div className="ci-empty-text">즐겨찾기한 메시지가 없습니다</div>
                                        <div className="ci-empty-sub" style={{fontSize:12,color:'var(--text-3)',marginTop:4,textAlign:'center',lineHeight:1.6}}>
                                            채팅 메시지를 꾹 누르면<br/>즐겨찾기에 추가할 수 있습니다
                                        </div>
                                    </div>
                                ) : starred.map((msg, idx) => {
                                    const isImg  = !!msg.imageData
                                    const isFile = !!msg.fileName
                                    const isLink = !isImg && !isFile && msg.text && msg.text.startsWith('http')
                                    ...
                                    return (
                                        <div key={msg.id ?? idx} style={{
                                            margin:'8px 14px', padding:'13px 14px',
