...
//  AI 채팅 창 — 로컬 키워드 매칭 방식 (API 비용 없음)
// ══════════════════════════════════════════════════════════════
...
const QA_DATABASE = [
    ...
    {
        keywords: ['병원', '아프', '아파', '몸'],
        answer: '병원 관련 수어예요! 🏥\n\n① \'아프다\': 아픈 부위에 손을 대고 찡그린 표정\n② \'병원\': 두 손 검지로 적십자(+) 표시\n③ \'의사\': 손목 맥박 재는 동작\n\n병원에서 청각장애인은 필담, 수어 통역 서비스를 요청할 권리가 있어요.',
    },
]
...
//  메인 컴포넌트
// ══════════════════════════════════════════════════════════════
export default function AIChat({ onClose, loggedIn = false, displayName = '' }) {
    const [messages, setMessages] = useState([])
    const [input,    setInput]    = useState('')
    const [sending,  setSending]  = useState(false)
    const [speaking, setSpeaking] = useState(null) // 재생 중인 메시지 id
                        ...
                        <div className="aic-bubble-wrap">
                            <div className={`aic-bubble ${msg.role === 'user' ? 'aic-bubble-user' : 'aic-bubble-ai'}`}>
                                {msg.loading ? (
                                    ...
                                    <span className="aic-bubble-text">{msg.content}</span>
                                )}
                            </div>
